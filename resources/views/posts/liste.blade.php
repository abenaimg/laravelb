...
@if(Auth::user()->admin)
    <div class="btn-group pull-right">
         {!! link_to_route('user.index', trans('blog.liste'), [], ['class' => 'btn btn-info']) !!}
    </div>
@endif
@endsection
		...
		<article class="row bg-primary">
			<div class="col-md-12">
				...
				<section>
					...
					@endif
					<em class="pull-right">
						<span class="glyphicon glyphicon-pencil"></span> {{ $post->user->name  . ' ' . trans('blog.on') . ' ' . $post->created_at  }}<!--$post->created_at->format('d-m-Y') -->
					</em>
				</section>
			</div>
		</article>
